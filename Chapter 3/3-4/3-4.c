...
/*
 * It does not handle it because the largest negative number does not
 * have a positive equivalent. -n where n is the largest negative number
 * gives n; because n is now negative n%10 gives negative values instead
 * of the desired positive ones. If we take the asbolute value of n%10
 * we get the correct value and character.
 */
...
/* itoa:  comvert n to characters in s */
void itoa(int n, char s[])
{
    int i, sign;
